...
lead: Short answers to the questions that come up most often.
breadcrumb: Documentation
breadcrumb_href: /documentation/
# Renders a filter box; worth it on a page of forty-odd questions.
searchable: true
---
...
== Who governs Debezium, and how is it licensed?

Debezium is an open source project hosted by the https://www.commonhaus.org[Commonhaus Foundation].
The code is licensed under the Apache License 2.0; the link:/license/[licence page] covers the full details, including the website and the Debezium marks.
Project roles and how decisions get made are described in our link:/community/governance/[governance model], and the link:/foundation/faq/[Debezium at Commonhaus FAQ] answers questions about the move to the foundation.
...
== What does a Kafka deployment of Debezium look like?

A running Debezium system consists of several pieces.

A cluster of http://kafka.apache.org[Apache Kafka] brokers provides the persistent, replicated, and partitioned transaction logs where Debezium records all events and from which applications consume all events.
Kafka relies upon either a small cluster of http://zookeeper.apache.org[Zookeeper] nodes (in versions prior to 4.0) or KRaft (Kafka Raft) for consensus and to manage responsibilities of each broker.
The number of Kafka brokers depends largely on the volume of events, the number of database tables being monitored, and the number of applications that are consuming the events.

Each Debezium connector captures changes from one database cluster/server, and connectors are configured and deployed to a cluster of Kafka Connect services that ensure that each connector is always running, even as Kafka Connect service instances leave and join the cluster.
Each Kafka Connect service cluster (a.k.a., group) is independent, so it is possible for each group within an organization to manage its own clusters.
Multiple Kafka Connect service clusters can share a single cluster of Kafka brokers.

Applications connect to Kafka directly and consume the events within the appropriate topics, or organizations use Kafka Connect consumers (sink) connectors to read from Kafka and write events to a target system.
...
There are two ways to reset the offsets depending on your Kafka Connect version:

* xref:#reset-offsets-kafka-3.6[]
* xref:#reset-offsets-kafka-3.5[]

[id=reset-offsets-kafka-3.6]
=== With Kafka Connect 3.6 or later

Kafka Connect introduced a set of dedicated REST API endpoints for managing connector offsets as part of KIP-875.
These endpoints simplify the process of removing connector offsets with a few simple REST endpoint calls.

Step 1 - Stop the connector::
In order to cleanly remove a connector's offsets, the connector must be in a *stopped* state.
To place the connector in the stopped state, issue an HTTP `PUT` request to the `/connectors/<connector-nane>/stop` endpoint.
+
```bash
curl -X PUT -H "Content-Type: application/json" \
  "https://<host>:<port>/connectors/<connector-name>/stop"
```
+
[NOTE]
====
Please wait for the connector to gracefully shutdown before proceeding.
====

Step 2 - Reset the offsets::
This next step involves effectively removing the existing offsets for the connector.
This is a destructive action and cannot be undone.
To remove the offsets, issue an HTTP `DELETE` request to the `/connectors/<connector-name>/offsets` endpoint.
+
```bash
curl -X DELETE -H "Content-Type: application/json" \
  "https://<host>:<port>/connectors/<connector-name>/offsets"
```
+
[WARNING]
Before performing this step, the connector must be in a *stopped* state, see Step 1.
If the connector is not stopped, the resetting the offsets will have unpredictable results.

Step 3 - Remove schema history topic::
When a connector no longer has offsets, it's expected that a schema history topic also does not exist.
To guarantee that the connector starts without an error, it's important to also remove the schema history topic.
This applies generally to all Debezium connectors except PostgreSQL.

Step 4 - Restart the connector::
In Step 1, the connector was placed in a *stopped* state and was not removed.
In order to restart the connector so that it begins capturing changes, issue an HTTP `PUT` request to the `/connectors/<connector-nane>/resume` endpoint.
+
```bash
curl -X PUT -H "Content-Type: application/json" \
  "https://<host>:<port>/connectors/<connector-name>/resume"
```
+
[NOTE]
If you chose to undeploy the connector in Step 1 rather than using the Stop endpoint, then in this step you should redeploy the connector instead of using the Resume endpoint.

[id=reset-offsets-kafka-3.5]
=== With Kafka Connect 3.5 or earlier
