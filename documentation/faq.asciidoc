...
== Why JSON messages don't contain schema?

If you are using `JsonConverter` to convert and serialize the messages the schema is not by default included in the message.
The schemas enabled using `schemas.enable` configuration parameter set either at worker level (e.g. `connect-distibuted.properties`):
