...
Several configuration options like `table.include.list` that defines a set of regular expressions are not case sensitive when applied to identifiers.

If your environment requires case-sensitive matching (e.g. two identifiers are differing in letter-case only), then you can use regular expression flag `(?-i)` for a given expression to mandate case-sensitive matching.
