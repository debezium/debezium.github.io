...
The very first step to reduce the message size is externalization of the schema to a separate system and keeping only the _link_ to the schema definition.
https://github.com/Apicurio/apicurio-registry[Apicurio Registry] can be used as the external storage for the schemas.
Regardless of message format used (JSON, Avro, Protocol Buffers) the schema is no longer stored directly in the message.
It uses the fact that many events in a row will use the same schema, and by registering those schemas in the separate API/Schema registry instance, it can place into each serialized event a small _identifier_ of the schema version used by the message.
The converters and the API/Schema registry can work together to track the history of each schema over time.

In the consumer, the same converter decodes the event, reads the identifier of the schema version used by that message, if it hasn't yet seen that schema version downloads the schema from the API/Schema registry instance, and finally uses that schema to decode the payload of the event.
Again, many events in sequence will share the same schema (and schema version), so most of the time the converter can simply decode the raw event into the same schema and payload expected by the consumer.

The next step in reducing the message size is replacement of JSON format (human-readable but quite verbose) with a https://martin.kleppmann.com/2012/12/05/schema-evolution-in-avro-protocol-buffers-thrift.html[very compacted binary form].
Apicurio provides Kafka Connect converters and Kafka serializers and deserializers for Avro and Protocol Buffers binary formats in addition to extended JSON format.
In all cases the message schema is externalized to the API/Schema registry and in case of Avro and Protocol Buffers the messages are converted into a binary packed representation with size significantly smaller than JSON format.

[[how-do-i-use-confluents-avro-converter]]
[[using-the-avro-converter]]
[[avro-converter]]
== Is it possible to use Confluent's Avro Converter?
If you are deploying Debezium connectors to a Kafka Connect worker service, simply make sure the Confluent Avro Converter JARs are available and configure the worker service to use the Avro Converter.
You will, for example, need to point the converter to your Confluent Schema Registry.
Then, simply deploy the Debezium connectors (or really, any other Kafka Connect connectors) to your worker service.
See link:/docs/configuration/avro/[Avro Serialization] for a detailed description of how to use the Confluent Avro converter.
