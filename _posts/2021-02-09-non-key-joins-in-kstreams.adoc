...
By choosing the foreign key column as the message key for change events, the two table streams could be joined without the need for re-keying the topic manually.
...
If you aren’t using Prometheus, you have a few options like:
...
Consider a case when the database connection isn't available. A simple healthcheck can be added as follows:
[source,java]
----
@Readiness
@ApplicationScoped
public class ConnectionHealthCheck implements HealthCheck {
    ...
    public HealthCheckResponse call() {
        ...
        HealthCheckResponseBuilder checkResponseBuilder = HealthCheckResponse.named("Database connection health check");

        try {
            simulateDatabaseConnection();
            checkResponseBuilder.up();
        } catch (DebeziumException exception) {
            checkResponseBuilder.down()
                    .withData("error", exception.getMessage());
        }

        return checkResponseBuilder.build();
    }

    private void simulateDatabaseConnection() {
        if (!databaseUp) {
            throw new RuntimeException("Cannot connect database");
        }
    }
}
----

Using this extension will expose a health check via HTTP under `q/health` which shows the status of the pipeline and the missing topics.
...
=== References
 ...
 - https://github.com/confluentinc/kafka-streams-examples[Confluent Kafka Tutorials]
