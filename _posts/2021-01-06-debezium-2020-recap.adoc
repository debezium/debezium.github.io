...
* Besides these new connectors, each of the releases brought a wide range of new features; some of the things I'm most excited about are http://127.0.0.1:4000/documentation/reference/1.2/operations/debezium-server.html[Debezium Server] for integrating Debezium with message infrastructure like Apache Pulsar, AWS Kinesis, Google Cloud Pub/Sub, and Azure Event Hubs, the http://127.0.0.1:4000/documentation/reference/1.1/integrations/outbox.html[Quarkus extension] for implementing the outbox pattern, 
the new http://127.0.0.1:4000/documentation/reference/connectors/oracle.html#_logminer[LogMiner-based connector implementation] for ingesting change events from Oracle,
