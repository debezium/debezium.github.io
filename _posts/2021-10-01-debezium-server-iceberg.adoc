...
In this blog post we will see a simple way to build Data lake. The solution is using realtime data pipeline, supporting ACID transaction, SQL updates and highly scalable. And its not requiring to run Apache Kafka or Apache Spark applications to run the data feed.
