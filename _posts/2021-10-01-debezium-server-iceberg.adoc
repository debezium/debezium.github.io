...
Additional data lake layers needs to be periodically updated with new data. The easiest way of doing it is using SQL update or SQL delete insert statements. This sql operations are supported by  https://iceberg.apache.org/spark-writes/[Iceberg]:
