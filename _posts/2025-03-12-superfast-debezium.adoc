...
Alright, let’s start building the application! All the proposed code is available on https://github.com/debezium/debezium-examples/tree/main/quarkus-native[GitHub].
...
2. **Address Reflection in Debezium**: All the classes managed with the `ServiceLoader` should be registered using Quarkus and GraalVM guidelines. Furthermore, additional configurations (like a transformation) that inject classes should https://github.com/debezium/debezium-examples/pull/374#issuecomment-2724221196[take into account the native build].
...
- https://quarkus.io/guides/config[Configure a Quarkus application]
