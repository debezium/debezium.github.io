...
Service meshes such as https://istio.io/[Istio] come in helpful here, by providing capabilities like circuit breakers,
...
by having the order service propagate events through a durable message log such as http://kafka.apache.org/[Apache Kafka],
...
E.g. consider a data warehouse which should keep information about all the orders ever placed, or some full-text search functionality on orders based on https://www.elastic.co/products/elasticsearch[Elasticsearch].
...
also a record describing the event to be sent is inserted into that `Events` table.
...
As we'll see, though, the pattern can be implemented in a very elegant and efficient way using change data capture and Debezium.
In the following, let's explore how.

== An Implementation Based on Change Data Capture

link:/blog/2018/07/19/advantages-of-log-based-change-data-capture/[Log-based Change Data Capture] (CDC) is a great fit for capturing new entries in the events table and stream them to Apache Kafka.
...
Debezium is used to capture new events in the outbox table of the order service's database and propagate them to Apache Kafka.
...
In order to "send" events to the outbox, code in the order service could in general just do an `INSERT` into the `Events` table.
...
which will do the required `INSERT` into the events table.
...
Now let's take a look at the code that consumes any fired `ExportedEvent` and does the corresponding write to the events table:
