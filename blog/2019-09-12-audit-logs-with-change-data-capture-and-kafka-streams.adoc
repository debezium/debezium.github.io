...
You can find a https://github.com/debezium/debezium-examples/tree/master/auditlog[complete example] with all the components and instruction for running them on GitHub.
...
class ChangeEventEnricher implements Transformer<JsonObject, JsonObject, KeyValue<JsonObject, JsonObject>> {
    ...
    public void init(ProcessorContext context) {
        ...
        context.schedule(Duration.ofSeconds(1), PunctuationType.WALL_CLOCK_TIME, ts -> enrichAndEmitBufferedEvents()); // <4>
    }
     ...
     */
    private KeyValue<JsonObject, JsonObject> enrichWithTxMetaData(JsonObject key, JsonObject changeEvent) { // <1>
...
----
<1> When a vegetables change event arrives, look up the corresponding metadata in the statestore of the
...
<3> Before actually getting to the incoming event, all buffered events are processed; only if all could be enriched, the incoming event will be processed, too
...
_Many thanks to https://twitter.com/hpgrahsl[Hans-Peter Grahsl], https://twitter.com/hashhar[Ashhar Hasan], Jiri Pechanec and https://twitter.com/crancran77[Chris Cranford] for their feedback while working on this post and the accompanying example code!_
