...
Running asynchronously, the CDC process can extract the change data without impacting OLTP transactions, not missing any event, no matter what its origin is and whether it was caused by a bulk operation.
