...
We have developed a Debezium connector for usage with link:https://www.ibm.com/analytics/db2[Db2] with
...
Chage Data Capture (CDC) systems propagate mofifications at the source tables made via Data Manipulation Language (DML) operations such as INSERT, DELETE etc. They do not explicitly handle changes to the source table made via Data Definition Language (DDL) operations such as TRUNCATE, ALTER etc. It is not really clear what the behaviour of Debezium should be made when a DDL change occurs. We are
looking at exploring what the Debezium model should be for changes of this sort.


== Conclusion

Database replication tools have been around for as long as database systems themselves; these have been designed for the mirroring of
one database system into other by system administrators for load balancing and fault tolerance.
While it is attractive to assume a new system built completely from scratch it will almost certainly be necessary to interact with existing relational database systems for some considerable time. 
Debezium is a promising framework for connecting existing enterprise data systems within
a hybrid cloud data orchestration system.
