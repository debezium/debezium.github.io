...
The key to this is the `exactly_once` value of the `processing.guarantee` property given in _application.properties.
This ensures a transactionally consistent processing, e.g. in the aforementioned scenario,
after a restart the original change event would be handled again, and the buffer state would look like it did before the event was processed for the first time.

Consumers of the enriched vegetable events should apply an isolation level of `read_committed`;
otherwise they may see uncommitted and thus duplicate messages in case of an application crash after a buffered event was forwarded but before it was remove from the buffer.
===============================
