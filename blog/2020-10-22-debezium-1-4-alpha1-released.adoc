...
* Allow re-snapshot of specific tables
...
We are excited that the Vitess engineers decided to build a CDC solution based on Debezium and to https://www.github.com/debezium/debezium-connector-vitess[open-source it] under the Debezium umbrella.
This connector is released in *incubating* state in Debezium 1.4.

A blog post with more details around this connector will follow every soon;
...
It's common practice when a specific table(s) need to be re-snapshotted that a user would need to create an additional connector specifically to re-publish the snapshot of those tables.
While this process works well, it is tedious and often not very straight forward.

A new connector configuration, `snapshot.include.collection.list`,
solves this problem by allowing users to reconfigure an existing connector so that it performs a snapshot on select tables/collections.
This means that users no longer need to consider spinning up a new connector for this step anymore.
