...
As long as the data changes are done through Hibernate, this is nothing to worry about: the ORM will update the cache automatically.

Things get tricky, though, when bypassing the application, e.g. when modifying records directly in the database.
Hibernate then has no way of knowing that the cached data has become stale, and it's necessary to invalidate the affected items explicitly.
...
This uses the Caffeine library for maintaining a short-lived in-memory cache of transaction ids encountered by the application.
As data change events arrive in near-realtime, the TTL can be rather short
...
The next step is to retrieve the current transaction id whenever a request is processed by the application and register it with `KnownTransactions`.
This should happen once per transaction.
There are multiple ways for implementing this logic; in the following a Hibernate `FlushEventListener` is used for this purpose:
...
Hibernate event listeners are not subject to dependency injection via CDI.
...
[NOTE]
===============================
The session / entity manager can also be flushed manually, in which case `txid_current()` function would be invoked multiple times.
...
In this blog post we've explore how Debezium and change data capture can be employed to invalidate application-level caches after external data changes.
...
If you got any feedback on this idea, please post a comment below or come to our https://groups.google.com/forum/#!forum/debezium[mailing list].
