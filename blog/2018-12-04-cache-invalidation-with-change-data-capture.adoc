...
by employing Debezium and its change data capture (CDC) capabilities, you can track data changes in the database itself and react to any applied change.
...
++++
<div class="imageblock centered-image">
    <img src="/images/cache_invalidation_architecture.png" style="max-width:100%; margin-bottom:20px; margin-top:20px;" class="responsive-image" alt="Example domain model">
</div>
++++

While this doesn't come with the scalability and fault tolerance provided by Apache Kafka,
it's a great fit for the requirements at hand.
...
For the use case at hand it is solely needed to receive data change events while the application is running, and using the embedded engine enables exactly that.
...
There's no need to process any change events occurring while the connector is not running
...
The first thing needed for that is a class for the transaction book keeping:
