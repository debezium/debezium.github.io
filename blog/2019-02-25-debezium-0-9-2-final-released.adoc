...
Overall, https://issues.jboss.org/issues/?jql=project%20%3D%20DBZ%20AND%20fixVersion%20%3D%200.9.2.Final[18 issues] were resolved.
...
(https://issues.jboss.org/browse/DBZ-1141[DBZ-1141], https://issues.jboss.org/browse/DBZ-1149[DBZ-1149])
* The connector won't fail any longer when encountering a change to a row with an unaltered TOAST column value
(https://issues.jboss.org/browse/DBZ-1146[DBZ-1146])

Also the link:/docs/connectors/mysql/[Debezium MySQL connector] saw a number of fixes:

* The connector works correctly now when using GTIDs and `ANSI_QUOTES` SQL mode (https://issues.jboss.org/browse/DBZ-1147[DBZ-1147])
* The new Antlr-based DDL parsers can handle column names that are key words such as `MEDIUM` (https://issues.jboss.org/browse/DBZ-1150[DBZ-1150])
* `TIME` columns with a default value larger than 23:59:59 can be exported now (https://issues.jboss.org/browse/DBZ-1137[DBZ-1137])
...
This has been resolved now, so the connector archive only contains the actually needed JARs and thus is much smaller (https://issues.jboss.org/browse/DBZ-1138[DBZ-1138]).
...
(https://issues.jboss.org/browse/DBZ-1130[DBZ-1130])
* The MongoDB connector now has the long-awaited snapshotting mode `NEVER` (https://issues.jboss.org/browse/DBZ-867[DBZ-867]),
...
We've also upgraded the binlog client used by the MySQL connector to version 0.19.0 (https://issues.jboss.org/browse/DBZ-1140[DBZ-1140]),
which fixes a bug that had caused exceptions during rebalancing the connector before (https://issues.jboss.org/browse/DBZ-1132[DBZ-1132]).
