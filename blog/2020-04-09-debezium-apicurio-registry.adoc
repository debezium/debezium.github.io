= Using the Apicurio API and Schema Registry with Debezium
jpechane
...
To consume the messages on the sink side we can, for example, demonstrate an Elasticsearch sink connector. The sink configuration will be again extended only with converter configuration and the sink connector can consume from Avro-enabled topics without any other changes needed.
