...
Also the Debezium Docker images have been updated do that version (https://issues.jboss.org/browse/DBZ-305[DBZ-305]).
...
Debezium provides a new https://github.com/debezium/debezium/blob/master/debezium-core/src/main/java/io/debezium/transforms/UnwrapFromEnvelope.java[single message transformation] (https://issues.jboss.org/projects/DBZ/issues/DBZ-226[DBZ-226]).
...
If you are doing the Debezium link:/docs/tutorial/[tutorial], you will like the new https://github.com/debezium/debezium-examples/tree/master/tutorial[Docker Compose set-up] provided in the examples repo (https://issues.jboss.org/browse/DBZ-127[DBZ-127]).
...
* Snapshot consistency wasn't guaranteed before in some corner cases (https://issues.jboss.org/browse/DBZ-210[DBZ-210]); that's fixed now
* DEC and FIXED types supported in the DDL parser (https://issues.jboss.org/browse/DBZ-359[DBZ-359]; thanks to https://github.com/ooooorz[Liu Hanlin]!)
* UNION clause supported for ALTER TABLE (https://issues.jboss.org/browse/DBZ-346[DBZ-346])

For the **MongoDB connector**, the way of serializing ids into the key payload of CDC events has changed (https://issues.jboss.org/browse/DBZ-306[DBZ-306]).
...
Another nice improvement for this connector is support for SSL connections (https://issues.jboss.org/projects/DBZ/issues/DBZ-343[DBZ-343]).

Finally, the **Postgres connector** learned some new tricks, too:

* Support for variable-width numeric columns (https://issues.jboss.org/browse/DBZ-318[DBZ-318])
* Views won't stop the connector any more (https://issues.jboss.org/browse/DBZ-319[DBZ-319])
* Warnings and notifications emitted by the server are correctly forwarded to the log (https://issues.jboss.org/browse/DBZ-279[DBZ-279])

Please refer to the http://debezium.io/docs/releases/#release-0-6-0[changelog] for an overview of all the 20 issues fixed in Debezium 0.6.0.

== What's next?

High on our agenda is exploring support for Oracle (https://issues.jboss.org/browse/DBZ-20[DBZ-20]).
