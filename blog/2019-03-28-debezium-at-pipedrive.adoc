...
As Debezium connector can’t automatically recover from MySQL connection issues, it needs another management script to restart the task. At first we used an external cron-like script for this, but now as Node.js is already running in container, we delegated this responsibility there as well.

The logic is very simple - every 30 seconds it connects to the Kafka Connect REST API running in the local container and retrieves the connector status. If the connector or task is in FAILED state, it tries to restart it by posting to _connectors/jobname/tasks/1/restart_ up to 5 times. If it still doesn’t work or there’s UNASSIGNED state, a graceful shutdown of the Docker container is initiated.

== Securing the Kafka Connect REST API
Because the Kafka Connect REST API had no built in security (in the beginning of 2018 at least), anybody could query and see Kafka cluster and MySQL credentials in plaintext. To prevent such leak, we’ve added a simple Node.js HTTP proxy script with HTTP basic auth on a different port and only expose this port to the outside. Once auth is passed this Node.js proxy just forwards all the requests to Kafka Connect on local port 8080.
https://issues.jboss.org/browse/DBZ-702[DBZ-702]

== Monitoring with Prometheus/Grafana
Debezium exposes nice JMX variables, but to query them using the Prometheus scraper, we added the Prometheus exporter into the Debezium docker image. This library has worked without problems https://github.com/prometheus/jmx_exporter[jmx_prometheus_javaagent] and we've most Debezium and Kafka Connect prefixed variables as Prometheus metrics.
