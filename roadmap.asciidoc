...
* Further improve and expand Oracle connector based on LogMiner
* DB history topic compaction tool
* Explore "interactive" snapshots based on commands in the signalling table (allowing to re-snapshot specific tables, resume snapshots after a restart, change include/exclude filters, begin streaming while snapshots are running, etc.
