...
[NOTE]
====
As of Debezium 0.8 all time related types that does not have associated timezone are supposed to have a implicit timezone which is the timezone configured for the database.
The timezone is configured via `server.zone.offset` configuration option.

The time and timestamp values are normalized and converted to UTC time. So if the database is running in Central European timezone the time is shifted back by one hour to the UTC value.
====
...
This value is used to normalize timestamps without timezones to the value as defined by server timezone.
