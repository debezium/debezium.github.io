...
[NOTE]
====
PostgreSQL connector uses a schema information that is a part of events icoming from a logical decoder.
The only exception is the information about what columns compose the primary key as this information is obtained from the JDBC metadata (side channel).
If a primary key composition changes (by adding, removing or renaming columns) then there exists a very slight risk of an unfortunate timing when the primary key information from JDBC will not be synchronized with the metadata in logical decoding event and a small amount of messages will be created with an outdated key.
If this happens then a restart of the connector and a reprocessing of the messages will fix the issue.
To prevent the issue completely it is recommended to synchronize the primary key update operation with Debezium roughly using following sequence of operations
...
* Put the database or the application into a read/write state and start Debezium
====
