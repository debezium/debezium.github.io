...
[NOTE]
====
As of Debezium 0.10, we introduced support for PostgreSQL 10+ Pgoutput logical replication.  This decoder mode does not require the installation of any additional libraries as the connector is able to interpret the raw replication event stream into change events directly.
====
...
Before using the Debezium PostgreSQL connector to monitor the changes committed on a PostgreSQL server, first decide which logical decoder method you intend to use.  If you plan not to use the native PgOutput logical replication stream support, then you will need to install the logical decoding plugin into the PostgreSQL server.  Afterward enable a replication slot, and configure a user with sufficient privileges to perform the replication.

Note that *if your database is hosted by a service* such as https://www.heroku.com/postgres[Heroku Postgres] you may be unable to install the plugin. If so, if you're using PostgreSQL 10+ then you'll need to use the PgOutput decoder support to monitor your database.  If that isn't an option, you'll be unable to monitor your database with Debezium.
...
[NOTE]
====
As of Debezium 0.10, the connector now supports PostgreSQL 10+ logical replication streaming using _pgoutput_.  This means that a logical decoding output plug-in is no longer necessary and changes can be emited directly from the replication stream by the connector.
====
...
If you are using one of the supported external link:#output-plugin[plugin] and it has been installed, configure the server to load the plugin at startup:
...
0.7.0 and later
|`decoderbufs`
|The name of the Postgres link:#output-plugin[logical decoding plugin] installed on the server. Supported values are either `decoderbufs`, `wal2json`, `wal2json_rds`, or `pgoutput`. +
There are two new options supported since 0.8.0.Beta1

* `wal2json_streaming`
* `wal2json_rds_streaming`.

There is a new option supported since 0.10.0.Beta3
...
This publication is created at start-up if it does not already exist to include _all tables_.  Debezium will then use its own white-/black- list filtering capabilities to limit change events to the specific tables of interest if configured.

If the publication already exists and configured with a subset of tables, Debezium will instead use the publication as defined.
|`database.hostname`
|
