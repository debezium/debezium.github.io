...
[NOTE]
====
As of Debezium 0.8 all temporal column types without a timezone are interpreted as per the timezone configured via `server.zone.offset` configuration option.

Using that timezone, the time and timestamp values are converted to UTC time and represented as such in emitted CDC messages.
So if for instance the database is running in Central European timezone, temporal values without an explicit timezone are shifted back by one hour to the corresponding UTC value.
====
...
0.8.0 and later
|
| Specifies timezone offset of the timezone where database server is located.
This value is used to normalize timestamps without timezones into UTC, based on the timezone offset of the specified server timezone.
Defaults to UTC.
