...
0.7.0 and later
|`false`
|When Debezium meets a field of a datatype that is not explicitly known by Debezium then the field is omitted from the output event.
A user can prefer to keep the field and send it downstream to clients in the opaque binary representation so the clients will decode it themselves.
Set to `false` to filter unknown data out of events and `true` to keep them in binary format.

_Note: The clients risk backward compatibility issues. When the datatype is later supported by Debezium as it will be sent downstream in a logical type, not the binary as before._
