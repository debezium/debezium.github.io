...
Starting with MySQL 5.6 the MySQL server can be configured to include the original SQL statements with each binlog event. This feature can be enabled by setting the https://dev.mysql.com/doc/refman/5.7/en/replication-options-binary-log.html#sysvar_binlog_rows_query_log_events[binlog_rows_query_log_events] global variable to `ON`.  *Note that if you're using an earlier version of MySQL, you will not be able to enable this feature.*
...
* the value for https://dev.mysql.com/doc/refman/5.7/en/replication-options-binary-log.html#sysvar_binlog_rows_query_log_events[`binlog_rows_query_log_events`] can be set to `on` or `ON` to enable support for capturing the original SQL statement in the binlog entry.
