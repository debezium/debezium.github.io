...
However, when `time.precision.mode` configuration property is set to `adaptive`, then the connector will base the literal type and semantic type for the temporal types on the column's data type definition so that the event's data exactly represents the values in the database:
