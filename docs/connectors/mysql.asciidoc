...
[NOTE]
====
As of Debezium 0.8 all temporal column types without a timezone are interpreted as per the timezone configured for the database.
At the start time Debezium queries the database for its timezone settings, but it is possible to configure this setting explicitly using the `database.serverTimezone` configuration option.
