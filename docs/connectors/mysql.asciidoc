...
0.9.0 and later
|`latest`
| When set to 'latest', when the connector sees new GTID channel, it will start consuming the channel from the server latest executed transaction position. If set to 'earliest' the connector starts reading that channel from first available (not purged) gtid position on the server. 'Earliest' is useful when you have active-passive mysql setup where debezium is connected to master, in this case during failover the slave with new UUID (and GTID channel) starts receiving writes.
