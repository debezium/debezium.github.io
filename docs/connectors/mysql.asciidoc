...
|The `length` schema parameter contains an integer representing the number of bits. The resulting `byte[]` will contain the minimum number of bytes required to store the bits in little-endian form.
