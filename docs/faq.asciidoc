...
you need to specify your docker VM's host system's IP address or host name as `ADVERTISED_HOST_NAME` for the Kafka container: `docker run -it --rm --name kafka -p 9092:9092 -e ADVERTISED_HOST_NAME=<%YOUR_DOCKER_VM_HOST_NAME%> --link zookeeper:zookeeper debezium/kafka:{debezium-docker-label}`.
