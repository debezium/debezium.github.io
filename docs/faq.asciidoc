...
The latest version of Debezium includes support for monitoring link:/docs/connectors/mysql[MySQL database servers], link:/docs/connectors/mongodb[MongoDB replica sets or sharded clusters], and link:/docs/connectors/postgresql[PostgreSQL servers]. Support for other DBMSes will be added in future releases. See our link:/docs/connectors/[list of connectors].
...
Our current Docker images for Kafka Connect include the Avro Converter as an option. We'll likely change this in the future.
