...
== Unexpected EventDataDeserializationException, even when history topic is valid.. ==

When you run into random deserialization exceptions around 1 minute after starting connector, but schema topic seems correct and no OOM and error has "caused by" "EOF" or "connection reset" in logs. Something like this:
...
Then updating these mysql server global properties will fix it.
```
