...
Before we can use Docker, make sure that Docker is https://docs.docker.com/engine/installation/[installed and running]. You can either manually start the Docker daemon or configure it to run automatically on startup. In either case, the _Docker host_ is your local machine.
