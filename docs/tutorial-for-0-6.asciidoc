...
Debezium 0.6.2 includes support for monitoring MySQL database servers with its link:/docs/connectors/mysql/[MySQL connector], and this is what we'll use in this demonstration. Support for other DBMSes will be added in future releases.
...
[TIP]
====
Debezium 0.6.2 requires Kafka Connect 0.11.0.1, and in this tutorial we also use version 0.11.0.1 of the Kafka broker. Check the http://kafka.apache.org/documentation.html[Kafka documentation] about compatibility between different versions of Kafka Connect and the Kafka broker.
====
...
This shows that we're running Kafka Connect version 0.11.0.1. Next, check the list of connectors, again using your IP address in place of `localhost`:
